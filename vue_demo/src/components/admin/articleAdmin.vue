...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: "作者", value: "writer" },
        { text: "文章类型", value: "a_type" },
        { text: "发表时间", value: "w_time" },
  ...
  methods: {
    ...
    search() {
      this.$Axios({
        url: "/admin/getActicleByTypeAndChar",
        method: "get",
        data: {
          currPage: 0,
          pageNum: 100,
    ...
    deleteItemConfirm() {
      this.$commonAxios({
        url: "/admin/delActicle",
        method: "get",
        data: {
          a_id: this.editedItem.a_id,
        },
        success: (res) => {
          console.log(res);
        },
    ...
    save() {
      if (this.editedIndex > -1) {
        ...
        this.$Axios({
          url: "/admin/upActicleData",
          method: "post",
          data: {
            a_id: this.editedItem.a_id,
            a_type: this.editedItem.a_type,
  ...
  computed: {
    // 对话框标题
    formTitle() {
      return this.editedIndex === -1 ? "编写文章" : "文章信息编辑";
    },
  },
  watch: {
    // 是否显示对话框
